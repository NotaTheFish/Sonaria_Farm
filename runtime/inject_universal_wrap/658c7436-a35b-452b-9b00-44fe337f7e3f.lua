...
local _deadRestartLastAt = 0
local _deadRestartLogLastAt = 0
...
local function tryClickCreaturePlayButton(creatureName)
    ...
    local function clickGuiHard(gui, label)
        if not gui or not gui:IsA("GuiObject") or not gui.Visible then
            return false
        end
        local pos = gui.AbsolutePosition + (gui.AbsoluteSize / 2)
    ...
    local function pressActionButton(btn, label)
        ...
        -- В CoS поверх action-кнопок есть UpperLabel/ImageLabel с большим ZIndex,
        -- которые могут перехватывать мышь и не пробрасывать до кнопки.
        ...
        local ok = false
        -- 1) Прямые UI-сигналы (если доступны у эксплойта)
        pcall(function()
        ...
        -- 2) Нативная активация GuiButton
        pcall(function()
            btn:Activate()
            ok = true
        end)
        -- 3) Фолбэк: жёсткая серия VIM-кликов
        if clickGuiHard(btn, label) then
            ok = true
        end
    ...
    local function findActionButtonInCard(slotCardHint, token)
        ...
        -- Сначала пробуем строгий путь из SaveSelectionGui:
        -- <slot>.InnerFrame.CreatureFrame.ButtonFrame.(PlayButton|RestartButton|ReviveButton)
        local inner = slotCardHint:FindFirstChild("InnerFrame")
        local creatureFrame = inner and inner:FindFirstChild("CreatureFrame")
        local buttonFrame = creatureFrame and creatureFrame:FindFirstChild("ButtonFrame")
        if buttonFrame then
            local preferred = nil
            if token == "play" then
            ...
            if
                preferred
                and preferred:IsA("GuiButton")
                and preferred.Visible
                and preferred.Active ~= false
                and preferred.AbsoluteSize.X > 2
                and preferred.AbsoluteSize.Y > 2
            then
                return preferred
            end
            -- Fallback внутри ButtonFrame только (без глобальных эвристик по карточке).
        ...
        local candidates = {}
        for _, q in ipairs(slotCardHint:GetDescendants()) do
            if
                q:IsA("GuiButton")
                and q.Visible
                and q.Active ~= false
    ...
    local function tryHandleDeadCreature(slotCardHint, slotSelected)
        -- По ТЗ: если в выбранной карточке нет Play — жмём Restart в этой же карточке.
        if not slotCardHint or type(slotCardHint.GetDescendants) ~= "function" then
            return false
        end
        if not slotSelected then
            -- Даже если флаг выбора не проставился, карточка уже вычислена по имени существа — пробуем restart в ней.
        end
        local now = tick()
        if now - _deadRestartLastAt < 2.5 then
            return true
        end
        ...
        if now - _deadRestartLogLastAt >= 6 then
            _deadRestartLogLastAt = now
            local rp = "?"
            pcall(function()
                rp = slotRestart:GetFullName()
            end)
            log("DEAD restart target: " .. tostring(rp))
        end
        ...
        wait(0.12)
        pressActionButton(slotRestart, "Dead creature restart (slot2)")
        playUiImportant("DEAD creature: restart clicked in selected slot")
        return true
    end
