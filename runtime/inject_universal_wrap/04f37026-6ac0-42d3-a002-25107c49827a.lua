...
local function tryClickCreaturePlayButton(creatureName)
    ...
    local function playUiImportant(msg)
        local now = tick()
        if now - _playUiLastLogAt >= 2 then
            _playUiLastLogAt = now
            log(msg)
        end
    end
    ...
    local function trySelectCreatureSlotByName(creatureLower)
        ...
        if not slotsFrame or not slotsFrame:IsA("GuiObject") then
            return false
        end
        for _, card in ipairs(slotsFrame:GetChildren()) do
            if card:IsA("GuiObject") and card.Visible and card.AbsoluteSize.X > 40 and card.AbsoluteSize.Y > 60 then
                ...
                if string.find(blob, creatureLower, 1, true) then
                    clickGui(card, "Creature slot select")
    ...
    if #candidates == 1 then
        if creatureName ~= "" then
            playUiImportant("PlayButton: одна видимая кнопка, жмём после выбора слота " .. creatureName)
        end
