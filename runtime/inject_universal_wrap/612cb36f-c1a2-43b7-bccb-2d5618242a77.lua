...
local function tryEnterWorldFromSlotUi(maxSeconds, creatureName)
    ...
    while tick() - t0 < maxSeconds do
        ...
        if character then
            -- Character exists, but we need Humanoid to confirm we're actually in-game
            local hum = character:FindFirstChildOfClass("Humanoid")
            if hum then
                humanoid = hum
                log("Персонаж загружен после экрана слотов (Humanoid ok, Health=" .. tostring(hum.Health) .. ")")
                return true
            end
            -- Character without Humanoid — wait up to 10s for it to appear
            local humWait = character:WaitForChild("Humanoid", 10)
            if humWait then
                humanoid = humWait
                log("Персонаж загружен после экрана слотов (Humanoid appeared, Health=" .. tostring(humWait.Health) .. ")")
                return true
            end
            -- Humanoid didn't appear — character is likely a menu/transitional state; keep clicking Play
            log("Character без Humanoid после 10s — продолжаем Play loop")
        end
        local now = tick()
...
local function doSuicideVolcano()
    ...
    while tick() < deathTimeout do
        ...
        if not humanoid or humanoid.Health <= 0 then
            log("Суицид в лаве: персонаж мёртв")
            break
        end
        -- Re-acquire HRP (character ref may change after refreshCharacterRefs)
    ...
    if humanoid and humanoid.Health > 0 then
        log("WARN: лава не убила за 15с, форсируем Health=0")
        pcall(function() humanoid.Health = 0 end)
        wait(2)
    end

    -- Wait for respawn with timeout (CoS may show slot selection screen)
    wait(3)
    local respawnDeadline = tick() + 15
    while tick() < respawnDeadline do
        character = player.Character
        if character then
    ...
    refreshCharacterRefs()
    log("Суицид в лаве: респавн завершён (char=" .. tostring(character ~= nil) .. ")")
end
...
handlers.farm = function()
    ...
    while true do
        ...
        refreshCharacterRefs()
        if not humanoid or humanoid.Health <= 0 then
            log("Респавн — перезапуск существа через tryEnterWorldFromSlotUi")
...
local function main()
    log("=== UNIVERSAL SONARIA BOT STARTED === [v9-humanoid-fix]")
