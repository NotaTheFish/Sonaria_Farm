...
local function tryClickCreaturePlayButton(creatureName)
    ...
    local function tryHandleDeadCreature(slotCardHint, slotSelected)
        ...
        if promptConfirmed then
            ...
            log("DEAD creature: restart CONFIRMED via prompt RunButton")
            wait(1.5)
            return true
        end

        -- Шаг 3а: пробуем remote напрямую — самый надёжный способ, не зависит от GUI/DevConsole.
        if creatureName ~= "" then
            local remoteOk, remoteErr = tryRestartSlotViaRemote(creatureName, slotCardHint)
            if remoteOk then
                _deadRestartLastAt = now
                log("DEAD creature: restarted via RestartSlotRemote (direct)")
                wait(1.5)
                return true
            end
            log("DEAD restart: remote direct attempt: " .. tostring(remoteErr))
        end

        -- Шаг 3б: жмём RestartButton в карточке слота → откроется модалка подтверждения.
        ...
        if not slotRestart then
            if _deadRestartFailLogLastAt == nil or (now - _deadRestartFailLogLastAt) >= 8 then
                ...
                log("DEAD restart: no RestartButton found in card, no remote, no prompt")
            end
            return false
        end
        ...
        log("DEAD restart: pressActionButton returned " .. tostring(clicked))
        wait(0.3)
        ...
        log("DEAD restart: prompt not found after click, all methods exhausted -> will retry next cycle")
        return false
    end
    ...
    for _ = 1, 4 do
        if selectedCard then
            ...
            warn("[v2] playInCard=" .. tostring(playInCard) .. " creatureName=" .. tostring(creatureName))
            if playInCard then
                return clickGui(playInCard, "PlayButton click")
            end
            warn("[v2] calling tryHandleDeadCreature now")
            local deadOk, deadErr = pcall(tryHandleDeadCreature, selectedCard, slotSelected)
            warn("[v2] tryHandleDeadCreature returned ok=" .. tostring(deadOk) .. " err=" .. tostring(deadErr))
            if deadOk and deadErr == true then
                return true
            end
            if not deadOk then
                warn("[v2] tryHandleDeadCreature ERROR: " .. tostring(deadErr))
            end
...
local function main()
    log("=== UNIVERSAL SONARIA BOT STARTED === [v2-restart-fix]")
