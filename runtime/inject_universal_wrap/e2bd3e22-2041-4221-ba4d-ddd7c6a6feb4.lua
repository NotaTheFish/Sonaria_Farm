...
local function tryClickCreaturePlayButton(creatureName)
    ...
    local function tryHandleDeadCreature(slotCardHint, slotSelected)
        ...
        local beforeVisible = slotRestart.Visible
        local beforeActive = slotRestart.Active
        local clicked = pressActionButton(slotRestart, "Dead creature restart (slot)")
        wait(0.12)
        local clicked2 = pressActionButton(slotRestart, "Dead creature restart (slot2)")
        wait(0.08)

        -- Считаем успех только если Restart перестал быть доступным после нажатия
        -- (открылась модалка / кнопка исчезла / стала неактивной).
        local afterRestart = findActionButtonInCard(slotCardHint, "restart")
        local consumed = false
        if not afterRestart then
            consumed = true
        else
            consumed = (afterRestart ~= slotRestart)
                or (afterRestart.Visible ~= beforeVisible)
                or (afterRestart.Active ~= beforeActive)
        end
        ...
        if _deadRestartFailLogLastAt == nil or (now - _deadRestartFailLogLastAt) >= 4 then
            ...
            log(
                "DEAD restart press not confirmed: clicked1="
                    .. tostring(clicked)
                    .. " clicked2="
                    .. tostring(clicked2)
                    .. " after_visible="
                    .. tostring(afterRestart and afterRestart.Visible or "nil")
                    .. " after_active="
                    .. tostring(afterRestart and afterRestart.Active or "nil")
                    .. " -> retry"
            )
        end
        return false
