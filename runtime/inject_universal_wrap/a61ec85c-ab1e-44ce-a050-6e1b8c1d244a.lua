...
local function tryClickCreaturePlayButton(creatureName)
    ...
    local function tryConfirmRestartPrompt()
        ...
        for _, pName in ipairs(promptNames) do
            local f = notifFrame:FindFirstChild(pName)
        ...
        -- Fallback: ищем любой видимый фрейм с "restart" в имени
        ...
        -- Fallback 2: любой видимый фрейм с RunButton внутри
        if not promptFrame then
            for _, child in ipairs(notifFrame:GetChildren()) do
                if child:IsA("GuiObject") and child.Visible and child:FindFirstChild("RunButton", true) then
                    promptFrame = child
                    matchedName = child.Name .. " (has RunButton)"
                    break
                end
            end
        end

        -- Диагностика: если промпт не найден, дампим содержимое NotificationFrame
        if not promptFrame then
            if _promptDumpLastAt == nil or (tick() - _promptDumpLastAt) >= 5 then
                ...
                if #parts > 0 then
                    log("NotificationFrame children: " .. table.concat(parts, " | "))
                else
                    log("NotificationFrame has NO children")
                end
            end
            return false
        end
        ...
        if clickTarget and clickTarget:IsA("GuiButton") then
            log("Restart prompt found: " .. matchedName .. ", clicking " .. tostring(clickTarget.Name))
            pressActionButton(clickTarget, "Restart confirm")
            if clickTarget.Name ~= "RunButton" then
                pcall(function()
                    local parentBtn = clickTarget.Parent
                    if parentBtn and parentBtn:IsA("GuiButton") then
                        pressActionButton(parentBtn, "Restart confirm (parent)")
                    end
                end)
            end
            return true
        ...
        log("Restart prompt '" .. matchedName .. "' RunButton found but no clickable target")
        return false
    end

    -- Fallback: ищем кнопку "Restart" во ВСЁМ PlayerGui (на случай, если промпт не в NotificationsGui).
    local function tryConfirmRestartAnywhere()
        local found = {}
        pcall(function()
            for _, desc in ipairs(pg:GetDescendants()) do
                if desc:IsA("GuiButton") and desc.Visible and isAncestryVisible(desc) then
                    local nm = string.lower(tostring(desc.Name or ""))
                    local hasRestartText = false
                    pcall(function()
                        for _, t in ipairs(desc:GetDescendants()) do
                            if (t:IsA("TextLabel") or t:IsA("TextButton")) then
                                local tx = string.lower(tostring(t.Text or ""))
                                if tx == "restart" then
                                    hasRestartText = true
                                end
                            end
                        ...
                        if desc:IsA("TextButton") then
                            local tx = string.lower(tostring(desc.Text or ""))
                            if tx == "restart" then
                                hasRestartText = true
                            end
                        end
                    end)
                    if hasRestartText and nm ~= "restartbutton" then
        ...
        for _, btn in ipairs(found) do
            local path = "?"
            pcall(function() path = btn:GetFullName() end)
            if string.find(path, "Notification", 1, true) or string.find(path, "Prompt", 1, true) or string.find(path, "Dialog", 1, true) then
                log("Restart confirm (anywhere): clicking " .. path)
                pressActionButton(btn, "Restart confirm (anywhere)")
                return true
            end
        end
        if #found > 0 then
            local path = "?"
            pcall(function() path = found[1]:GetFullName() end)
            log("Restart confirm (anywhere fallback): clicking " .. path)
            pressActionButton(found[1], "Restart confirm (anywhere fallback)")
            return true
        end
        return false
    end
    ...
    local function tryHandleDeadCreature(slotCardHint, slotSelected)
        ...
        local promptConfirmed = tryConfirmRestartPrompt() or tryConfirmRestartAnywhere()
        ...
        for attempt = 1, 12 do
            ...
            -- Fallback: ищем кнопку Restart где угодно в PlayerGui
            local anyConfirm = tryConfirmRestartAnywhere()
            if anyConfirm then
                _deadRestartLastAt = now
                log("DEAD creature: restart CONFIRMED via anywhere search (attempt " .. attempt .. ")")
                wait(1.5)
                return true
            end
            wait(0.3)
...
local function main()
    log("=== UNIVERSAL SONARIA BOT STARTED === [v5-confirm-fix]")
