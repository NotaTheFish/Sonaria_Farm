...
local function tryClickCreaturePlayButton(creatureName)
    ...
    local function slotCardFromPlayButton(btn)
        local p = btn
        for _ = 1, 24 do
            if not p or not p.Parent then
                break
            end
            if p.Name == "CreatureFrame" then
                return p
            end
            if p.Parent.Name == "SlotsFrame" then
                return p
            end
            p = p.Parent
        end
    ...
    playUiLog("PlayButton видимых кнопок: " .. tostring(#candidates) .. " creature=" .. tostring(creatureName))

    -- Строго ищем кнопку Play у слота, где текст содержит имя существа.
    if creatureName ~= "" then
        for _, btn in ipairs(candidates) do
            local card = slotCardFromPlayButton(btn)
            if card then
                local blob = collectTextBlob(card)
                if card.Parent then
                    blob = blob .. " " .. collectTextBlob(card.Parent)
                end
                if string.find(blob, creatureName, 1, true) then
                    playUiLog("Found creature slot: " .. creatureName)
                    return clickGui(btn, "PlayButton click (matched creature slot)")
                end
            end
        end
        if #candidates == 1 then
            -- В некоторых версиях CoS имя существа не лежит рядом с PlayButton.
            playUiLog("PlayButton: имя не найдено, но кнопка одна — жмём её")
            return clickGui(candidates[1], "PlayButton click (single fallback)")
        end
        local now = tick()
        if now - _playUiSlotMissLastAt >= 4 then
