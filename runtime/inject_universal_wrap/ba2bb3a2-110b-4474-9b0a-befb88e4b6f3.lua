...
local function tryClickCreaturePlayButton(creatureName)
    ...
    local function pressActionButton(btn, label)
        ...
        -- 4) mouse1click (WeAreDevs / часть эксплойтов)
        pcall(function()
            if type(mouse1click) == "function" then
                local pos = btn.AbsolutePosition + (btn.AbsoluteSize / 2)
                local inset = GuiService:GetGuiInset()
                mouse1click(pos.X, pos.Y + inset.Y)
                ok = true
            end
        end)
        -- 5) VIM (игнорируем F9 — см. clickGuiHard)
    ...
    -- CoS: после клика RestartButton открывается модалка NotificationsGui > NotificationFrame > RestartCreature(NoMutations).
    -- Внутри: RunButton (Frame) → UpperLabel (ImageButton) — подтверждение рестарта.
    local function tryConfirmRestartPrompt()
        local notifGui = pg:FindFirstChild("NotificationsGui")
        if not notifGui then
            return false
        end
        local notifFrame = notifGui:FindFirstChild("NotificationFrame")
        if not notifFrame then
            return false
        end
        local promptNames = { "RestartCreature", "RestartCreatureNoMutations" }
        for _, pName in ipairs(promptNames) do
            local promptFrame = notifFrame:FindFirstChild(pName)
            if promptFrame and promptFrame:IsA("GuiObject") and promptFrame.Visible then
                local runBtn = promptFrame:FindFirstChild("RunButton", true)
                if runBtn then
                    local upperLabel = nil
                    if runBtn:IsA("GuiButton") then
                        upperLabel = runBtn
                    else
                        upperLabel = runBtn:FindFirstChild("UpperLabel")
                    ...
                    if upperLabel and upperLabel:IsA("GuiButton") then
                        log("Restart prompt found: " .. pName .. ", clicking RunButton")
                        local ok = pressActionButton(upperLabel, "Restart confirm (RunButton)")
                        if ok then
                            return true
                        end
                    end
    ...
    local function tryRestartSlotViaRemote(targetCreatureName)
        local sonar = nil
        pcall(function()
        ...
        -- Ищем имя слота по имени существа в данных игрока.
        local pw = nil
        pcall(function()
            pw = sonar("PlayerWrapper")
        end)
        if not pw then
            return false, "no PlayerWrapper"
        end
        local playerData = nil
        pcall(function()
            playerData = pw.GetPlayerData(player)
        end)
        if not playerData then
            pcall(function()
                playerData = pw.GetData(player)
            end)
        end

        local slotName = nil
        local targetLower = string.lower(targetCreatureName or "")
        if targetLower ~= "" and playerData then
            local creatures = nil
            pcall(function()
                creatures = playerData:FindFirstChild("Creatures") or playerData:FindFirstChild("Slots")
            end)
            if creatures then
                for _, slot in ipairs(creatures:GetChildren()) do
                    local dino = slot:FindFirstChild("Dino")
                    local health = slot:FindFirstChild("Health")
                    if dino and dino:IsA("StringValue") then
                        local dinoLower = string.lower(tostring(dino.Value or ""))
                        if dinoLower == targetLower or string.find(dinoLower, targetLower, 1, true) then
                            local isDead = health and type(health.Value) == "number" and health.Value <= 0
                            if isDead then
                                slotName = slot.Name
                                break
                            end
        ...
        log("RestartSlotRemote: slot=" .. tostring(slotName) .. " pcall_ok=" .. tostring(ok) .. " res=" .. tostring(res))
        if ok and res then
            return true
        end
    ...
    local function tryHandleDeadCreature(slotCardHint, slotSelected)
        ...
        -- Шаг 4: если модалка подтверждения уже открыта — жмём RunButton в ней.
        local promptConfirmed = tryConfirmRestartPrompt()
        if promptConfirmed then
            _deadRestartLastAt = now
            playUiImportant("DEAD creature: restart CONFIRMED via prompt RunButton")
            wait(1.5)
            return true
        end

        -- Шаг 3: жмём RestartButton в карточке слота.
        local slotRestart = findActionButtonInCard(slotCardHint, "restart")
        if not slotRestart then
            -- Нет ни кнопки Restart, ни открытой модалки → пробуем remote напрямую.
            if creatureName ~= "" then
                local remoteOk, remoteErr = tryRestartSlotViaRemote(creatureName)
                if remoteOk then
                    _deadRestartLastAt = now
                    playUiImportant("DEAD creature: restarted via RestartSlotRemote (no GUI)")
                    wait(1.5)
                    return true
                end
                if _deadRestartFailLogLastAt == nil or (now - _deadRestartFailLogLastAt) >= 8 then
                    _deadRestartFailLogLastAt = now
                    log("DEAD restart: no RestartButton, remote fallback failed: " .. tostring(remoteErr))
                end
            end
            return false
        end

        if _deadRestartLogLastAt == nil or (now - _deadRestartLogLastAt) >= 6 then
            _deadRestartLogLastAt = now
            local rp = "?"
            pcall(function()
        ...
        local clicked = pressActionButton(slotRestart, "Dead creature restart (slot)")
        wait(0.25)

        -- После клика RestartButton CoS открывает модалку — ждём её и жмём RunButton.
        for attempt = 1, 6 do
            local confirmed = tryConfirmRestartPrompt()
            if confirmed then
                _deadRestartLastAt = now
                playUiImportant("DEAD creature: restart CONFIRMED via prompt (attempt " .. attempt .. ")")
                wait(1.5)
                return true
            end
            wait(0.2)
        ...
        -- GUI-подтверждение не сработало → fallback на remote.
        if creatureName ~= "" then
            local remoteOk, remoteErr = tryRestartSlotViaRemote(creatureName)
            if remoteOk then
                _deadRestartLastAt = now
                playUiImportant("DEAD creature: restarted via RestartSlotRemote (prompt fallback)")
                wait(1.5)
                return true
            end
        end

        if _deadRestartFailLogLastAt == nil or (now - _deadRestartFailLogLastAt) >= 4 then
            _deadRestartFailLogLastAt = now
            log(
                "DEAD restart press not confirmed: clicked="
                    .. tostring(clicked)
                    .. " restart_btn_visible="
                    .. tostring(slotRestart.Visible)
    ...
    for _ = 1, 4 do
        if selectedCard then
            ...
            if creatureName ~= "" and tryHandleDeadCreature(selectedCard, slotSelected) then
                return true
            end
