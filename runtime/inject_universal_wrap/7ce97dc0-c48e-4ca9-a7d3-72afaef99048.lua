...
local function tryClickCreaturePlayButton(creatureName)
    ...
    local function tryHandleDeadCreature(slotCardHint, slotSelected)
        ...
        if not slotSelected or not slotCardHint or type(slotCardHint.GetDescendants) ~= "function" then
            return false
        end
        local now = tick()
        ...
        local clicked = clickGui(slotRestart, "Dead creature restart (slot)")
        if not clicked then
            return false
        end
        _deadRestartLastAt = now
        wait(0.1)
        clickGui(slotRestart, "Dead creature restart (slot2)")
