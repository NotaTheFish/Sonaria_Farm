    ...
    local function trySelectCreatureSlotByName(creatureLower)
        ...
        for _, card in ipairs(slotsFrame:GetChildren()) do
            if card:IsA("GuiObject") and card.Visible and card.AbsoluteSize.X > 40 and card.AbsoluteSize.Y > 60 then
                ...
                local score = 0
                if creatureLower ~= "" and string.find(blob, creatureLower, 1, true) then
                    score = score + 100
                end
                ...
                local hasPlay = findPlayButtonUnder(card) ~= nil
                if hasPlay then
                    score = score + 5
                end
        ...
        if bestCard and bestScore > 0 then
            clickGui(bestCard, "Creature slot select")
            local inner = bestCard:FindFirstChild("InnerFrame")
