...
local function tryClickCreaturePlayButton(creatureName)
    ...
    local function findRestartButton(nodeHint)
        local all = findButtonsByToken("restart")
        if #all == 0 then
            return nil
        end
        if nodeHint then
            local near = findButtonNearNode(nodeHint, all)
            if near then
                return near
            end
        end
        local best = all[1]
        local bestZ = tonumber(best.ZIndex) or 0
        for i = 2, #all do
            local z = tonumber(all[i].ZIndex) or 0
            if z > bestZ then
                best = all[i]
                bestZ = z
            end
    ...
    local function tryHandleDeadCreature(nodeHint)
        -- DEAD-слот: сначала Restart на карточке, затем Restart в модалке подтверждения.
        local firstRestart = findRestartButton(nodeHint)
        if not firstRestart then
            return false
        end
        clickGui(firstRestart, "Dead creature restart (slot)")
        wait(0.2)
        local confirmRestart = findRestartButton(nil)
        ...
        playUiImportant("DEAD creature: restart clicked (no confirm dialog found)")
        return true
    end
    ...
    if creatureName ~= "" then
        if tryHandleDeadCreature(selectedNode) then
            return true
        end
    end
