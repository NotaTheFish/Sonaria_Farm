...
local PLAY_LOG_INTERVAL = tonumber(params.play_log_interval_seconds) or 3
...
local function playUiLog(msg)
    local now = tick()
    if now - _playUiLastLogAt >= PLAY_LOG_INTERVAL then
...
local function tryClickCreaturePlayButton(creatureName)
    ...
    local function findCreatureSlot(creatureLower)
        ...
        for _, d in ipairs(pg:GetDescendants()) do
            ...
            if d:IsA("TextLabel") or d:IsA("TextButton") then
                txt = string.lower(tostring(d.Text or ""))
            end
            if txt and txt ~= "" and string.find(txt, creatureLower, 1, true) then
                local p = d
                for _ = 1, 16 do
                    if not p or p == pg then
                        break
                    end
                    if p:IsA("GuiObject") then
                        local playBtn = findPlayButtonUnder(p)
                        if playBtn then
                            return p, playBtn
                        end
                    end
    ...
    local slotRoot, slotPlayButton = findCreatureSlot(creatureName)
    if slotRoot and slotPlayButton then
        -- Явно выбираем нужную карточку существа, затем жмём Play в этой же карточке.
        clickGui(slotRoot, "Creature slot select")
        wait(0.12)
        return clickGui(slotPlayButton, "PlayButton click (selected slot)")
    end
    ...
    playUiLog("PlayButton видимых кнопок: " .. tostring(#candidates))
    if #candidates == 1 then
        return clickGui(candidates[1], "PlayButton click (single candidate)")
    end
    for _, btn in ipairs(candidates) do
        ...
        for _ = 1, 18 do
            ...
            local blob = collectTextBlob(p)
            if creatureName == "" or string.find(blob, creatureName, 1, true) then
                if creatureName ~= "" then
                    if p and p:IsA("GuiObject") then
                        clickGui(p, "Creature slot select (by ancestor blob)")
                    end
                    wait(0.1)
                end
                return clickGui(btn, "PlayButton click (ancestor blob)")
            end
            p = p.Parent
        end
    end
    if #candidates > 0 and creatureName == "" then
        playUiLog("PlayButton: имя «" .. tostring(creatureName) .. "» не найдено в подписи слота — жмём первую кнопку")
        return clickGui(candidates[1], "PlayButton click (fallback first)")
    end
    if creatureName ~= "" then
        playUiLog("PlayButton: слот с именем «" .. tostring(creatureName) .. "» не найден")
    end
    return false
end

local function selectCreature(creatureName)
    playUiLog("Selecting creature: " .. creatureName)
    ...
    local success, selected = pcall(function()
        ...
        if spawnEvent and spawnEvent:IsA("RemoteEvent") then
            ...
            playUiLog("SpawnCreature:FireServer(" .. tostring(creatureName) .. ")")
        end
