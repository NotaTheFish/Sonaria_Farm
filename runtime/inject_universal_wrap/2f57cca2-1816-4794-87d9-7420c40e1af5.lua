...
local function tryClickCreaturePlayButton(creatureName)
    ...
    local function collectTextBlob(root)
        local parts = {}
        for _, q in ipairs(root:GetDescendants()) do
            if q:IsA("TextLabel") or q:IsA("TextButton") or q:IsA("TextBox") then
                parts[#parts + 1] = tostring(q.Text)
            end
        end
        parts[#parts + 1] = tostring(root.Name or "")
        return string.lower(table.concat(parts, " "))
    end
    local function trySelectCreatureSlotByName(creatureLower)
        local slotsFrame = pg:FindFirstChild("SlotsFrame", true) or pg:FindFirstChild("AllSlotsFrame", true)
        if not slotsFrame then
            return false
        end
        -- 1) Прямой поиск текстового узла с именем существа внутри карточек слотов.
        for _, node in ipairs(slotsFrame:GetDescendants()) do
            if node:IsA("TextLabel") or node:IsA("TextButton") or node:IsA("TextBox") then
                local txt = string.lower(tostring(node.Text or ""))
                if creatureLower ~= "" and txt ~= "" and string.find(txt, creatureLower, 1, true) then
                    local card = node
                    for _ = 1, 16 do
                        if not card or not card.Parent then
                            break
                        end
                        if card.Parent == slotsFrame then
                            break
                        end
                        card = card.Parent
                    end
                    if card and card:IsA("GuiObject") then
                        -- На части UI выбор срабатывает только при клике в конкретный узел текста/картинки.
                        clickGui(node, "Creature slot select (name)")
                        wait(0.05)
                        clickGui(card, "Creature slot select (card)")
                        local inner = card:FindFirstChild("InnerFrame")
                        if inner and inner:IsA("GuiObject") then
                            wait(0.05)
                            clickGui(inner, "Creature slot select (inner)")
                        end
                        for _, q in ipairs(card:GetDescendants()) do
                            if
                                (q:IsA("ImageLabel") or q:IsA("ImageButton") or q:IsA("Frame"))
                                and q.Visible
                                and q.AbsoluteSize.X > 50
                                and q.AbsoluteSize.Y > 50
                            then
                                wait(0.03)
                                clickGui(q, "Creature slot select (visual)")
                                break
                            end
                        end
                        if not _slotFoundLoggedOnce then
                            _slotFoundLoggedOnce = true
                            log("Слот существа найден и выбран: " .. tostring(creatureLower))
                        end
                        return true
                    end
                end
        ...
        -- 2) Fallback: лучшая непустая карточка (когда имя не является TextLabel в дереве).
        local bestCard = nil
        local bestScore = -999
        for _, card in ipairs(slotsFrame:GetChildren()) do
            if card:IsA("GuiObject") and card.Visible and card.AbsoluteSize.X > 40 and card.AbsoluteSize.Y > 60 then
                local blob = collectTextBlob(card)
                local score = 0
                if string.find(blob, "store", 1, true) then
                    score = score + 8
                end
                if string.find(blob, "gender", 1, true) or string.find(blob, "diet", 1, true) or string.find(blob, "age", 1, true) then
                    score = score + 6
                end
                if string.find(blob, "empty slot", 1, true) or string.find(blob, "create", 1, true) then
                    score = score - 10
                end
        ...
        if bestCard and bestScore > 0 then
            clickGui(bestCard, "Creature slot select (fallback card)")
            return true
        end
        return false
    end
    local function findPlayButtonUnder(node)
        for _, q in ipairs(node:GetDescendants()) do
            if
                q.Name == "PlayButton"
                and q:IsA("GuiButton")
                and q.Visible
    ...
    local function collectVisiblePlayButtons()
        local out = {}
    ...
    if creatureName ~= "" then
        -- В CoS часто видна только одна PlayButton для текущего выбранного слота.
        -- Поэтому выбор слота делаем отдельно через SlotsFrame по имени существа.
        trySelectCreatureSlotByName(creatureName)
        wait(0.1)
    end
    local candidates = collectVisiblePlayButtons()

    if #candidates == 1 then
        -- Небольшой локальный ретрай после выбора слота: PlayButton появляется с задержкой.
        if creatureName ~= "" then
            for _ = 1, 3 do
                if candidates[1].Visible then
                    break
                end
                wait(0.06)
            end
        end
        return clickGui(candidates[1], "PlayButton click")
    end

    if creatureName ~= "" then
        local slotsFrame = pg:FindFirstChild("SlotsFrame", true) or pg:FindFirstChild("AllSlotsFrame", true)
        if slotsFrame then
            for _, card in ipairs(slotsFrame:GetDescendants()) do
                if card:IsA("GuiObject") and card.Visible and card.AbsoluteSize.X > 40 and card.AbsoluteSize.Y > 60 then
                    local blob = collectTextBlob(card)
                    if string.find(blob, creatureName, 1, true) then
                        local btn = findPlayButtonUnder(card)
                        if btn then
                            playUiImportant("PlayButton: найден в карточке существа " .. creatureName)
                            return clickGui(btn, "PlayButton click (slot card)")
                        end
                    end
                end
            end
        ...
        local now = tick()
        if now - _playUiSlotMissLastAt >= 6 then
            _playUiSlotMissLastAt = now
            playUiImportant("PlayButton: слот «" .. tostring(creatureName) .. "» не найден")
            if now - _playUiSlotDumpLastAt >= 12 then
                _playUiSlotDumpLastAt = now
                local sf = pg:FindFirstChild("SlotsFrame", true) or pg:FindFirstChild("AllSlotsFrame", true)
                if sf then
                    local dumped = 0
                    for _, card in ipairs(sf:GetDescendants()) do
                        if
                            card:IsA("GuiObject")
                            and card.Visible
                            and card.AbsoluteSize.X > 120
                            and card.AbsoluteSize.Y > 150
                            and dumped < 6
                        then
                            local b = collectTextBlob(card)
                            if b ~= "" then
                                b = string.gsub(b, "%s+", " ")
                                if #b > 120 then
                                    b = string.sub(b, 1, 120) .. "..."
                                end
                                playUiImportant("slotdbg " .. tostring(dumped + 1) .. ": " .. card:GetFullName() .. " | " .. b)
                                dumped = dumped + 1
                            end
                        end
                    end
    ...
    if #candidates > 0 then
        playUiLog("PlayButton: creatureName empty, жмём первую кнопку")
        return clickGui(candidates[1], "PlayButton click (fallback first)")
    end
    return false
end
