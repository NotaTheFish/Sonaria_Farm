...
local function tryClickCreaturePlayButton(creatureName)
    ...
    local function tryHandleDeadCreature(nodeHint, slotCardHint, slotSelected)
        -- DEAD-слот: только Restart в уже выбранной карточке.
        ...
        local function hasDeadMarker(root)
            if not root or type(root.GetDescendants) ~= "function" then
                return false
            end
            for _, q in ipairs(root:GetDescendants()) do
        ...
        if not hasDeadMarker(slotCardHint) then
            return false
        end
        ...
        local slotRestart = restarts[1]
        if nodeHint and #restarts > 1 then
            local near = findButtonNearNode(nodeHint, restarts)
            if near then
                slotRestart = near
            end
        end
    ...
    if creatureName ~= "" then
        if tryHandleDeadCreature(selectedNode, selectedCard, slotSelected) then
            return true
        end
    end
