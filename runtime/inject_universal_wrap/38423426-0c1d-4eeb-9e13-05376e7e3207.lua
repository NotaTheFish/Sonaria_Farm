...
local _playUiDevConsoleWarnLastAt = 0
...
local function tryClickCreaturePlayButton(creatureName)
    ...
    local function clickGui(gui, label)
        ...
        pcall(function()
            ...
            for _, n in ipairs(cg:GetDescendants()) do
        ...
        if devConsoleOpen then
            local now = tick()
            if now - _playUiDevConsoleWarnLastAt >= 8 then
                _playUiDevConsoleWarnLastAt = now
                log("DevConsole открыт (F9): клики по GUI временно пропущены")
            end
            return false
        end
    ...
    local function tryHandleDeadCreature(nodeHint, slotCardHint)
        -- DEAD-слот: только Restart на карточке существа (подтверждение вынесем отдельным шагом).
        local now = tick()
        ...
        if slotCardHint and hasDeadMarker(slotCardHint) then
            slotRestart = findRestartButtonIn(slotCardHint)
        end
        if not slotRestart and nodeHint and nodeHint.Parent and hasDeadMarker(nodeHint.Parent) then
        ...
        playUiImportant("DEAD creature: restart clicked")
        return true
    end
