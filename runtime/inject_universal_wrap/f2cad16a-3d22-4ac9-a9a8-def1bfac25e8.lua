...
local function tryClickCreaturePlayButton(creatureName)
    ...
    local function click(btn)
        ...
        log("PlayButton click (Activate+VIM): " .. btn:GetFullName() .. " screen=(" .. tostring(x) .. "," .. tostring(y) .. ")")
        return true
    end
    local candidates = {}
    ...
    log("PlayButton видимых кнопок: " .. tostring(#candidates))
    ...
    if #candidates > 0 then
        log("PlayButton: имя «" .. tostring(creatureName) .. "» не найдено в подписи слота — жмём первую кнопку")
        return click(candidates[1])
    end
    return false
end
