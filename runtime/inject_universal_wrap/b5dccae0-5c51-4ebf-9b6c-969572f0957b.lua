...
local function tryClickCreaturePlayButton(creatureName)
    ...
    -- Путь из rbxlx/decompile: InnerFrame.CreatureFrame.ButtonsFrame.(PlayButton|RestartButton|...)
    local function findCreatureButtonsFrame(creatureFrame)
        if not creatureFrame then
            return nil
        end
        return creatureFrame:FindFirstChild("ButtonsFrame")
            or creatureFrame:FindFirstChild("ButtonFrame")
            or creatureFrame:FindFirstChild("Button")
    end
    ...
    local function findActionButtonInCard(slotCardHint, token)
        ...
        -- Строгий путь (rbxlx / decompile): InnerFrame.CreatureFrame.ButtonsFrame
        local inner = slotCardHint:FindFirstChild("InnerFrame")
        local creatureFrame = inner and inner:FindFirstChild("CreatureFrame")
        local buttonFrame = findCreatureButtonsFrame(creatureFrame)
        if buttonFrame then
            local preferred = nil
            if token == "play" then
