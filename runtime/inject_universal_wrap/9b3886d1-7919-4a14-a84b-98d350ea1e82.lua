...
local function doSuicideVolcano()
    ...
    -- Wait for death/respawn (CoS shows slot selection after creature death)
    wait(3)
    log("Суицид в лаве: ожидание респавна/слота...")
end
...
handlers.farm = function()
    ...
    while true do
        ...
        if not isCreatureAlive() then
            log("Существо мертво — перезапуск через tryEnterWorldFromSlotUi")
...
local function main()
    log("=== UNIVERSAL SONARIA BOT STARTED === [v10-cos-health]")
