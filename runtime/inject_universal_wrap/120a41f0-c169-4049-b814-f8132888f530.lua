...
local function tryClickCreaturePlayButton(creatureName)
    ...
    local function trySelectCreatureSlotByName(creatureLower)
        ...
        -- Надёжнее искать сам текст существа, затем подняться к карточке слота.
        for _, node in ipairs(pg:GetDescendants()) do
            if node:IsA("TextLabel") or node:IsA("TextButton") or node:IsA("TextBox") then
                ...
                if txt ~= "" and string.find(txt, creatureLower, 1, true) then
                    local p = node
                    for _ = 1, 24 do
                        if not p or not p.Parent then
                            break
                        end
                        local parentName = tostring(p.Parent.Name or "")
                        if parentName == "SlotsFrame" or parentName == "AllSlotsFrame" then
                            if p:IsA("GuiObject") and p.Visible then
                                -- Сначала клик по карточке, затем по более внутреннему фрейму (если есть).
                                clickGui(p, "Creature slot select")
                                local inner = p:FindFirstChild("InnerFrame")
                                if inner and inner:IsA("GuiObject") then
                                    wait(0.05)
                                    clickGui(inner, "Creature slot select (inner)")
                                end
                                playUiImportant("Слот существа найден и выбран: " .. creatureLower)
                                return true
                            end
                        end
                        p = p.Parent
